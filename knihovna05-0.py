from PyQt5.QtWidgets import (QApplication, QMainWindow, QMessageBox, QLineEdit, QTreeWidgetItem, QInputDialog, QGridLayout, QTreeWidget, QListWidget, QWidget, QHBoxLayout, QVBoxLayout, QLabel, QPushButton, QDesktopWidget, QStackedWidget, QButtonGroup, QSlider, QComboBox, QFrame)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        bottom_left_layout = QGridLayout(self.bottom_left_window) # Vytvoření layoutu
        self.bottom_left_window.setLayout(bottom_left_layout) # Nastavení layoutu widgetu
        ...
        # Vytvoření Entry pole PRO ZADÁVÁNÍ TEXTU
        self.kategorie_entry = QLineEdit(self.bottom_left_window) # TADY MÁ BÝT TENTO ŘÁDEK
        bottom_left_layout.addWidget(self.kategorie_entry, 0, 2) # Umístění vedle tlačítek
    ...
    def setup_buttons(self):
        ...
        def create_button_connection(button, action, entry=None):
            ...
            if action:
                try:
                    if button.text() in ("Přidat kategorii", "Přidat podkategorii"):
                        button.clicked.connect(create_button_connection(button, action, self.kategorie_entry))
                    else:
                        button.clicked.connect(create_button_connection(button, action))
